    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                assert (not self.ant.container) or \
                    (self.ant.container and self.ant.ant == None), \
                    "Two ants in same place."
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        """Indicates whether an Ant instance can contain another Ant instance or
        not.
        """
        return self.container
        """ESTO ESTÁ MAL, PORQUE PODRÍA MODIFICARSE LA PROPIEDAD, Y LA INSTANCIA
        SE COMPORTARÍA DEFORMA INCORRECTA. HABRÍA QUE DEVOLVER False.
        PERO LOS TESTS LO PIDEN ASÍ
        """
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 5
        def in_range():
            return self.min_range <= attack_range <= self.max_range

        place = self.place
        attack_range = 0
        while place != hive and attack_range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for i in range(len(place.bees) - 1, -1, -1):
                place.bees[i].reduce_armor(self.damage)
        # END Problem 4
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        """Damage all bees that pass through its place.
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower ant is a ThrowerAnt that can be in the water."""
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        """Indicates whether an Ant instance can contain another Ant instance or
        not.
        An Ant can contain another Ant if it is itself a container, and it does
        not currently contain another Ant, and the other ant is not a container.
        """
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 12
        """Damage all the bees in its place. Do the damage yielded by the ant
        contained in this BodyguardAnt plus the damage yielded by this ant.
