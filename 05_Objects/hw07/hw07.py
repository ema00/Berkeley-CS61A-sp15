...
def deep_map_mut(fn, link):
    ...
    while link != Link.empty:
        if isinstance(link.first, Link): #type(link.first) == Link:
...
def prune_min(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    [prune_min(b) for b in t.branches]
    if len(t.branches) < 2:
        return
...
def long_paths(tree, n):
    ...
    """Each path is a list of nodes. """
        if t.is_leaf():
            ...
            subpaths = []
            for b in t.branches:
                subpaths = subpaths + paths(b)
            root = t.root
            return [subpath.insert(0, root) for subpath in subpaths]

    def list_to_link(l):
        if l == []:
            return Link.empty
        else:
            return Link(l[0], list_to_link(l[1:]))

    paths_length_n = [p for p in paths(tree) if len(p) > n]
    return [list_to_link(p) for p in paths_length_n]
