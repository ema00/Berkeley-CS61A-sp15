...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        try:
            ...
            raise SchemeError('unknown identifier: {0}'.format(first))
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        values = operands.map(lambda expr: scheme_eval(expr, env))
        return scheme_apply(self, values, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        value = eval_all(expressions.second, env)
