    ...
    i_love_music
      ...
      trance
        darude
          sandstorm
    """
    pop = tree("pop", [tree("justin bieber", [tree("single", [tree("what do you mean?")])]), tree("2015 pop mashup")])
    trance = tree("trance", [tree("darude", [tree("sandstorm")])])
    return tree(username, [pop, trance])
...
def num_songs(t):
    ...
    if is_leaf(t):
        ...
        songs_per_branch = [num_songs(branch) for branch in branches(t)]
        return sum(songs_per_branch)
...
def find(t, target):
    ...
    """
    if is_leaf(t):
        return root(t) == target
    else:
        branches_have_target = [find(b, target) for b in branches(t)]
        return root(t) == target or any(branches_have_target)
